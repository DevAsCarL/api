            ...
            >
              <div class="form-check">
                <input
                  ...
                  :disabled="todo.task_status_id === 3"
                />
              </div>
              <div class="">{{ todo.task }}</div>
              <span class="badge bg-warning" v-if="todo.task_status_id == 1">IN PROGRESS</span>
              <span class="badge bg-success" v-else>COMPLETE</span>
              <div class="">
                <button
...
onMounted(() => {
  ...
  try {
    ...
    fetchTodos(userData.token, userData.id);
  } catch (error) {
...
const addTodo = async () => {
  ...
  try {
    await createTodo(todo, store.getters.user.token, store.getters.user.id);
    fetchTodos(store.getters.user.token, store.getters.user.id);
    newTodo.value = "";
...
const checkCompleted = async (index) => {
  ...
  try {
    ...
    const storedUserData = localStorage.getItem("userData");
    const userData = JSON.parse(storedUserData)
    const res = await getTodos(userData.token, userData.id);
    todoList.value = res.data;
...
const deleteTodoTask = async (index) => {
  try {
    ...
    await deleteTodo(todoList.value[index].id, store.getters.user.token);
...
const deleteAll = async () => {
  try {
    await deleteAllTodos(store.getters.user.token, store.getters.user.id);
    todoList.value = [];
